...
viscosity = Constant(1e21)  # Thermal diffusivity
mu = Constant(1e11) # Shear modulus in Pa
maxwell_time = viscosity / mu
dt = Constant(0.1*maxwell_time)  # Initial time-step
effective_viscosity = Constant(viscosity/(maxwell_time+0.5*dt))
prefactor_prestress = (maxwell_time - 0.5*dt)/(maxwell_time + 0.5*dt)

u_,p_ = z.split()
previous_stress = 2 * prefactor_prestress* effective_viscosity * sym(grad(u_old))
...
output_file = File("output_viscoelastic.pvd")
dump_period = dt
...
stokes_solver = StokesSolver(z, T, approximation, bcs=stokes_bcs, mu=mu, previous_stress=previous_stress, cartesian=True)
