...
m1 = Function(S, name='internal variable 1')
...
bulk_modulus = Function(W, name="bulk modulus").interpolate(2*shear_modulus)

viscosity = Function(W, name="viscosity")
initialise_background_field(viscosity, viscosity_values)

viscosity1 = Function(W, name="viscosity1").interpolate(0.5*viscosity)
viscfile = VTKFile("visc.pvd").write(viscosity, viscosity1)

m_file = VTKFile("m.pvd")
m_file.write(m)
m1_file = VTKFile("m1.pvd")
m1_file.write(m1)
# -
...
approximation1 = CompressibleInternalVariableApproximation(bulk_modulus, viscosity1, shear_modulus, g=g)
...
stokes_solver = CompressibleViscoelasticStokesSolver(z, [m, m1], density, approximation1,
                                         dt, bcs=stokes_bcs,)

m_solver = InternalVariableSolver(m, u, approximation,
                                         dt,  ImplicitMidpoint)

m1_solver = InternalVariableSolver(m1, u, approximation1,
                                         dt,  ImplicitMidpoint)
...
# +
# Create output file
output_file = VTKFile("output_burgers_2mu0_0.5visc.pvd")
output_file.write(z, displacement, viscosity, viscosity1)
